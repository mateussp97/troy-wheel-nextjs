...
const NewWheel = ({
  ...
  const names = useAtomValue(NameArrayAtom);
  const colors = useAtomValue(ColorArrayAtom);
  ...
  const spin = () => {
    ...
    if (timerHandle === 0) {
      ...
      maxSpeed = Math.PI / (names.length * 2 + Math.random());
      frames = 0;
  ...
  const onTimerTick = () => {
    ...
    if (duration < upTime) {
      ...
      if (winningSegment) {
        if (currentSegment === winningSegment && frames > names.length) {
          progress = duration / upTime;
          angleDelta =
  ...
  const drawNeedle = () => {
    ...
    let i =
      names.length - Math.floor((change / (Math.PI * 2)) * names.length) - 1;
    if (i < 0) i = i + names.length;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillStyle = "transparent";
    ctx.font = "bold 1.5em " + fontFamily;
    currentSegment = names[i];
    isStarted &&
